#!/usr/bin/env python3
"""Harness per la suite 07 (link error triage)."""
...
from utils.triage import ErrorTriage, link_rules
...
def main() -> int:
    ...
    triage = ErrorTriage(link_rules())

    cases = [
        ("T-0701-undefined-reference", "undefined_reference.log", "undefined_reference.json"),
        ("T-0702-ld-cannot-find", "ld_cannot_find.log", "ld_cannot_find.json"),
    ]
    ...
    print(f"[error_triage_link] Run artifacts in {run_paths.run_dir.relative_to(PROJECT_ROOT)}")
    return 0
