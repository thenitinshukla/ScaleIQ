#!/usr/bin/env python3
"""Harness per la suite 08 (GPU error triage)."""
...
from utils.triage import ErrorTriage, gpu_rules
...
def main() -> int:
    ...
    triage = ErrorTriage(gpu_rules())

    cases = [
        ("T-0801-nvcc-arch-mismatch", "nvcc_arch_mismatch.log", "nvcc_arch_mismatch.json"),
        ("T-0802-rocm-offload-arch", "rocm_offload_missing.log", "rocm_offload_arch.json"),
    ]
    ...
    for test_id, log_name, expected_name in cases:
        ...
        if 'arch' in outcome:
            outcome['detected_arch'] = outcome.pop('arch')
    ...
    print(f"[error_triage_gpu] Run artifacts in {run_paths.run_dir.relative_to(PROJECT_ROOT)}")
    return 0
